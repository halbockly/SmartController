...
class Switch:
    ...
    def Switching(self, param):                     # リクエストのJSON（{ kadenId:x, manipulateId:y }）を引数とする
        ...
        bool_status = self.priorConfirmation(kadenId, manipulateId)   # status.pyへ現在の家電のステータス確認
        if not bool_status:                                         # 既に求める状態になっている場合
            ...
            if result:                                          # 赤外線送信の成否
                st = Status()
                rewrite = st.changeStatusJson(kadenId, manipulateId)          # 成功：status.pyへのステータス書き換え依頼
    ...
    def priorConfirmation(self, kadenId, orderStatus):
        ...
        elif orderStatus == "2" and nowStatus == "2":
            return False                            # 既に求める状態になっている⇒戻り値false
        else:
